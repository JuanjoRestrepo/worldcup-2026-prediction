...
def _create_tournament_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["is_world_cup"] = df["tournament"].str.contains(
        "World Cup|WC|FIFA", case=False, na=False
    ).astype(int)
    ...
    df["is_continental"] = df["tournament"].str.contains(
        "Championship|Euro|COPA|Africa|Asian|Confederation",
        case=False, na=False
    ).astype(int)
...
def run_processing_pipeline(
    ...
    if not df_api.empty:
        ...
        logger.info(f"✅ Merged CSV + API: {len(df)} total unique matches (duplicates removed)")
    else:
        df = df_csv
    ...
    feature_cols = [col for col in df.columns if col not in 
                   ['homeTeam', 'awayTeam', 'homeGoals', 'awayGoals', 'date', 
                    'tournament', 'city', 'country', 'target', 'target_multiclass']]
