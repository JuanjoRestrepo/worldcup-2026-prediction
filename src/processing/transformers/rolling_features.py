...
def compute_rolling_features(df: pd.DataFrame, window: int = 5) -> pd.DataFrame:
    ...
    home_perspective = df[["date", "homeTeam", "homeGoals", "awayGoals"]].rename(
    ...
    away_perspective = df[["date", "awayTeam", "awayGoals", "homeGoals"]].rename(
    ...
    home_global = long_df.iloc[:len(df)][["global_avg_goals_for", "global_avg_goals_against"]].reset_index(drop=True)
    away_global = long_df.iloc[len(df):][["global_avg_goals_for", "global_avg_goals_against"]].reset_index(drop=True)
    
    df["home_global_avg_goals_last5"] = home_global["global_avg_goals_for"].values
    df["home_global_avg_conceded_last5"] = home_global["global_avg_goals_against"].values
    df["away_global_avg_goals_last5"] = away_global["global_avg_goals_for"].values
    df["away_global_avg_conceded_last5"] = away_global["global_avg_goals_against"].values
    ...
    long_df["result"] = np.where(
        long_df["goals_for"] > long_df["goals_against"], 1.0,
    ...
    df = df.drop(columns=["home_result", "away_result"], errors="ignore")
